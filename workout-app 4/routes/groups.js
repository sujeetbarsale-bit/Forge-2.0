...
// GET full detail for one group (member-only): members list + who's admin
router.get("/:groupId", requireAuth, requireMembership, (req, res) => {
  const db = req.db;
  const members = req.group.memberIds.map((uid) => {
    const u = db.users.find((x) => x.id === uid);
    return { id: uid, username: u ? u.username : "Unknown" };
  });
  res.json({
    id: req.group.id,
    name: req.group.name,
    ...
    isAdmin: req.group.createdBy === req.userId,
    members,
  });
});

// PATCH rename a group (admin/creator only)
router.patch("/:groupId", requireAuth, requireMembership, (req, res) => {
  if (req.group.createdBy !== req.userId) {
  ...
  writeDB(req.db);
  res.json({ success: true, name: req.group.name });
});

// POST remove a member from the group (admin/creator only)
router.post("/:groupId/remove-member", requireAuth, requireMembership, (req, res) => {
  if (req.group.createdBy !== req.userId) {
    return res.status(403).json({ error: "Only the group creator can remove members." });
  }
  const { userId } = req.body;
  if (!userId) return res.status(400).json({ error: "userId is required." });
  if (userId === req.group.createdBy) {
    return res.status(400).json({ error: "The group creator can't be removed." });
  }
  req.group.memberIds = req.group.memberIds.filter((id) => id !== userId);
  writeDB(req.db);
  res.json({ success: true });
});

// POST leave a group (any member except the creator, who should delete/transfer instead)
router.post("/:groupId/leave", requireAuth, requireMembership, (req, res) => {
  if (req.group.createdBy === req.userId) {
    return res.status(400).json({ error: "As the creator, you can't leave your own group." });
  }
  req.group.memberIds = req.group.memberIds.filter((id) => id !== req.userId);
  writeDB(req.db);
  res.json({ success: true });
});
